...
export default function CourseReviews({ reviews }) {
  ...
  const comparatorByFilterMethod = {
    'MostRecent': (x, y) => new Date(y.created_at).getTime() - new Date(x.created_at).getTime(),
    'LeastRecent': (x, y) =>
  ...
  useEffect(() => {
    let tempReviews = sortedReviews.concat();
    setSortedReviews(tempReviews.sort(comparatorByFilterMethod[filterMethod]));
  }, [filterMethod]);
